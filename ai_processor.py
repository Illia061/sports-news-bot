...
def create_enhanced_summary(article_data: Dict[str, Any]) -> str:
    ...
    prompt = f"""Ти редактор футбольних новин. Створи КОРОТКИЙ пост для Telegram каналу.

СУВОРІ ОБМЕЖЕННЯ:
- МАКСИМУМ 150 слів (це критично важливо!)
- Тільки найважливіші факти
- Українською мовою
- якщо є рейтинг у статті - публікуй максимум топ-5
- БЕЗ зайвих деталей та прикрас
- Якщо є пряма мова - максимум 1-2 речення
- НЕ повторюй заголовок на початку тексту!
- НЕ пиши "Заголовок:" або подібне

Використай так структуру відповіді, але  НЕ використовуй підзаголовки типу "Головний факт:", "Ключові деталі:" тощо в самому тексті!!!:
- Головний факт (1-2 речення)
- Ключові деталі (2-4 речення)


Заголовок: {title}
Текст: {content}

КОРОТКИЙ ПОСТ (максимум 150 слів):
"""
    try:
