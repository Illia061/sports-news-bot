...
async def main():
    ...
    # НОВАЯ ЛОГИКА: AI проверка на похожесть с недавними постами
    print(f"\n🔍 AI ПРОВЕРКА НА ДУБЛИКАТЫ")
    print("=" * 70)
    
    try:
        # Используем AI для более точной проверки дубликатов
        is_duplicate = check_content_similarity(processed_article, threshold=0.7)
        
        if is_duplicate:
            print("🚫 AI РЕШЕНИЕ: ПРОПУСКАЕМ публикацию")
            print("   Новость определена как дубликат недавно опубликованного контента")
            
            # Сохраняем информацию о пропущенной новости
            try:
                import json
                skip_log = {
                ...
                log_data.append(skip_log)
                log_data = log_data[-20:]  # Сохраняем последние 20
                
                with open(log_file, 'w', encoding='utf-8') as f:
                    json.dump(log_data, f, ensure_ascii=False, indent=2)
                
                print(f"📝 Информация о пропущенной новости сохранена")
                
            except Exception as e:
                print(f"⚠️ Не удалось сохранить лог пропуска: {e}")
            
            print("✅ Работа завершена - дубликат не опубликован")
            return
        else:
            print("✅ AI РЕШЕНИЕ: УНИКАЛЬНЫЙ КОНТЕНТ - публикуем")
            
    except Exception as e:
        print(f"⚠️ Ошибка AI проверки дубликатов: {e}")
        print("✅ Продолжаем публикацию (ошибка AI проверки)")
        import traceback
        print("🔍 Детали ошибки:")
        traceback.print_exc()
